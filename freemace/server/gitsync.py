...
log = logging.getLogger("freemace.gitsync")
...
def generate_ssh_key() -> str:
    """Generate an ed25519 SSH key pair for FreeMace. Returns the public key."""
...
def _rewrite_remote_for_ssh_alias(remote: str) -> str:
    ...
    if remote.startswith("git@github.com:"):
        return remote.replace("git@github.com:", "git@github.com-freemace:", 1)
    return remote
...
def get_remote(data_dir: Path) -> str | None:
    ...
    if rc == 0 and out:
        return out.replace("github.com-freemace", "github.com")
    return None
...
def git_init(data_dir: Path, remote: str | None = None):
    ...
    if not gitignore.exists():
        gitignore.write_text("# FreeMace data\n")
